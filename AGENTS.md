...
This canonical workflow tells any compatible agent how to take a topic and produce a Medium draft ready for Hari to review and publish.

**Read [`STANDARDS.md`](./STANDARDS.md) for the full HTML spec before writing any post.**

---

## External actions and approval gates

- A request to run this documented end-to-end workflow authorizes its scoped repository commit/push and Medium **draft import** after the mandatory local fact-check and file review.
- That request never authorizes clicking Medium **Publish**. Hari publishes or separately confirms a publication action.
- For tasks outside this end-to-end workflow, commits, pushes, imports, deployments, and publication require explicit user authorization.
- Record a canonical Medium URL and `published` status only after Hari confirms the story is published.
...
gh repo clone ylnhari/blog <repository-root>
